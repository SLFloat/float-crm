...
const SEED_PIPELINE_ENTRIES: PipelineEntry[] = [
...
export function AppProvider({ children }: { children: ReactNode }) {
  const [companies, setCompanies] = useState<Company[]>(() => loadFromStorage("crm_companies", SEED_COMPANIES));
  const [contacts, setContacts] = useState<Contact[]>(() => loadFromStorage("crm_contacts", SEED_CONTACTS));
  const [activities, setActivities] = useState<Activity[]>(() => loadFromStorage("crm_activities", SEED_ACTIVITIES));
  const [tasks, setTasks] = useState<Task[]>(() => loadFromStorage("crm_tasks", SEED_TASKS));
  const [pipelines, setPipelines] = useState<Pipeline[]>(() => loadFromStorage("crm_pipelines", SEED_PIPELINES));
  const [pipelineEntries, setPipelineEntries] = useState<PipelineEntry[]>(() => loadFromStorage("crm_pipeline_entries", SEED_PIPELINE_ENTRIES));
  const [deals, setDeals] = useState<Deal[]>(() => loadFromStorage("crm_deals", SEED_DEALS));

  useEffect(() => { localStorage.setItem("crm_companies", JSON.stringify(companies)); }, [companies]);
  ...
  useEffect(() => { localStorage.setItem("crm_pipelines", JSON.stringify(pipelines)); }, [pipelines]);
  useEffect(() => { localStorage.setItem("crm_pipeline_entries", JSON.stringify(pipelineEntries)); }, [pipelineEntries]);
